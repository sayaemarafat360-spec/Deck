...
fun VideoPlayerScreen(
    ...
    Box(Modifier.fillMaxSize().background(Color.Black)) {
        ...
        // Brightness drag zone (left half)
            ...
            .pointerInput(Unit) {
                awaitPointerEventScope {
                    while (true) {
                        ...
                        when (event.type) {
                            PointerEventType.Press -> dragStartY = pos.y
                            PointerEventType.Move  -> {
                                brightness = (brightness + (dragStartY - pos.y) / size.height * 0.5f).coerceIn(0f, 1f)
                                dragStartY = pos.y
                            }
                            else -> {}
                        }
                ...
                awaitPointerEventScope {
                    while (true) {
                        ...
                        when (event.type) {
                            PointerEventType.Press -> dragStartY = pos.y
                            PointerEventType.Move  -> {
                                volume = (volume + (dragStartY - pos.y) / size.height * 0.5f).coerceIn(0f, 1f)
        ...
        AnimatedVisibility(visible = showControls, enter = fadeIn(), exit = fadeOut()) {
            Box(Modifier.fillMaxSize().background(
                androidx.compose.ui.graphics.Brush.verticalGradient(
                    listOf(Color.Black.copy(0.7f), Color.Transparent, Color.Transparent, Color.Black.copy(0.7f))
                )
            )) {
                // Top bar
                Row(modifier = Modifier.fillMaxWidth().align(Alignment.TopStart)
                    ...
                    Box(modifier = Modifier.clip(RoundedCornerShape(8.dp))
                        ...
                        .clickable {
                            aspectIdx = (aspectIdx + 1) % aspectOptions.size
                            playerViewRef?.resizeMode = resizeModes[aspectIdx]
                        }.padding(horizontal = 10.dp, vertical = 5.dp)) {
                        Text(aspectOptions[aspectIdx], style = MaterialTheme.typography.labelSmall, color = Color.White)
                    }
                ...
                Row(modifier = Modifier.align(Alignment.Center),
                    ...
                    Box(modifier = Modifier.size(66.dp).clip(CircleShape)
                        ...
                        Icon(if (player.isPlaying) Icons.Filled.Pause else Icons.Filled.PlayArrow,
                ...
                Column(modifier = Modifier.fillMaxWidth().align(Alignment.BottomCenter)
                    ...
                    val position by produceState(0L) {
                        while (true) { value = player.currentPosition; delay(500) }
                    }
                    val duration = player.duration.coerceAtLeast(1L)
                    Slider(
                        value    = (position.toFloat() / duration).coerceIn(0f, 1f),
                        onValueChange = { player.seekTo((it * duration).toLong()) },
                        colors   = SliderDefaults.colors(activeTrackColor = NebulaViolet,
                            thumbColor = Color.White, inactiveTrackColor = Color.White.copy(0.3f))
                    )
                    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                        Text(formatVideoMs(position), style = MaterialTheme.typography.labelSmall, color = Color.White.copy(0.7f))
                        Text(formatVideoMs(duration), style = MaterialTheme.typography.labelSmall, color = Color.White.copy(0.7f))
                    }
                }
                ...
                Column(modifier = Modifier.align(Alignment.CenterStart).padding(start = 16.dp),
                    ...
                    Icon(Icons.Filled.BrightnessHigh, null, tint = Color.White.copy(0.8f), modifier = Modifier.size(18.dp))
                    Spacer(Modifier.height(4.dp))
                    Box(Modifier.width(4.dp).height(80.dp).clip(RoundedCornerShape(2.dp))
                        .background(Color.White.copy(0.2f))) {
                        Box(Modifier.fillMaxWidth().fillMaxHeight(brightness).align(Alignment.BottomCenter)
                ...
                Column(modifier = Modifier.align(Alignment.CenterEnd).padding(end = 16.dp),
                    horizontalAlignment = Alignment.CenterHorizontally) {
                    Icon(Icons.Filled.VolumeUp, null, tint = Color.White.copy(0.8f), modifier = Modifier.size(18.dp))
                    Spacer(Modifier.height(4.dp))
                    Box(Modifier.width(4.dp).height(80.dp).clip(RoundedCornerShape(2.dp))
                        .background(Color.White.copy(0.2f))) {
                        Box(Modifier.fillMaxWidth().fillMaxHeight(volume).align(Alignment.BottomCenter)
                            .background(Color.White.copy(0.8f)))
                    }
                }
            }
